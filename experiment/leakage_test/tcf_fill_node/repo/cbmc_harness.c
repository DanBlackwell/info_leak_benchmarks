...
int main(void) {
  ...
  if (tcf_fill_node(&skb1, &tp, fh, pid, seq, flags, event)) return 1;

  struct sk_buff skb2 = skb;
  if (tcf_fill_node(&skb2, &tp, fh, pid, seq, flags, event)) return 1;

  assert(skb1.len == skb2.len && !memcmp(skb1.data, skb2.data, skb1.len));
