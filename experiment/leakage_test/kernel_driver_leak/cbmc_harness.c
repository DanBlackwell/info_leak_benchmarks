...
static int sr9700_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	...
	while (skb->len > SR_RX_OVERHEAD) {
		...
		for (int i = 0; i < sizeof(sr_skb); i++) {
		  program_output[i] = ((uint8_t *)sr_skb)[i];
		  program_output_pos++;
		}
...
void cbmc_test() {
  ...
  skb.data = buf;
  skb.len = sizeof(buf);
  const struct usbnet emptyUsbnet = {0};
  struct usbnet dev = emptyUsbnet;

  sr9700_rx_fixup(&dev, &skb);
  char output1[sizeof(program_output)];
  int output1len = sizeof(program_output);
  ...
  skb2.len = sizeof(buf);
  dev = emptyUsbnet;
  sr9700_rx_fixup(&dev, &skb2);

  assert(program_output_pos == output1len);
